 ...
 * @subpackage Drivers
 */
class Files implements \Core\Storage\StorageInterface
{
	/**
	 * @var string
	 */
	...
	protected function generateFileName(\CAccount $oAccount, $iStorageType, $sKey, $bMkDir = false)
	{
		...
		switch ($iStorageType)
		{
			case \Core\Storage\Enumerations\StorageType::TEMP:
				$sTypePath = 'tmp';
				$sKeyPath = md5($sKey);
				$sKeyPath = substr($sKeyPath, 0, 2).'/'.$sKeyPath;
				break;
			case \Core\Storage\Enumerations\StorageType::USER:
				$sTypePath = 'data';
				$sKeyPath = md5($sKey);
				$sKeyPath = substr($sKeyPath, 0, 2).'/'.$sKeyPath;
				break;
			case \Core\Storage\Enumerations\StorageType::CONFIG:
				$sTypePath = 'cfg';
		...
		if ($bMkDir && !@is_dir(dirname($sFilePath)))
		{
			...
			{
				throw new \Core\Exceptions\Exception('Can\'t make storage directory "'.$sFilePath.'"');
			}
		}
