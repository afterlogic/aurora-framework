...
');
	...
	*/
			...
			echo '<div id="list_top_search" class="wm_contact_list_div_top">
					<form autocomplete="off" id="searchform" action="'; echo AP_INDEX_FILE; echo '?search" method="POST">' ;
						...
						echo '<div class="wm_toolbar_search_item" id="search_control">
							<span>'.CApi::I18N('ADMIN_PANEL/FORM_SEARCH').'</span>
	...
	{
		return (!empty($this->sSearchDesc)) ? '<br />'.CApi::I18N('ADMIN_PANEL/MSG_SEARCH_RESULTS').': "<b>'.$this->sSearchDesc.'</b>"
<br /><a href="'.AP_INDEX_FILE.'?reset_search">'.CApi::I18N('ADMIN_PANEL/MSG_SEARCH_RESET').'</a>' : '';
	}
...
';
			...
			{
				$sNameLoc = $sName;
                                if ($sName=="Type") $sNameLoc = CApi::I18N('ADMIN_PANEL/LIST_HEADER_TYPE');
...
<td id="'.$sName.'" class="'.$sClass.'" '.$sSizeStyle.' '.$sOnClick.'>
	<nobr>'.$sNameLoc.$sOrderTypeImg.'</nobr>
</td>
';
			}
