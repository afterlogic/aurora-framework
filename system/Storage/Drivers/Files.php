...
namespace SystemStorage\Drivers;
 ...
 * @subpackage Drivers
 */
class Files implements \SystemStorage\StorageInterface
{
	/**
	 * @var string
	 */
	...
	protected function generateFileName(\CAccount $oAccount, $iStorageType, $sKey, $bMkDir = false)
	{
		...
		switch ($iStorageType)
		{
			case \SystemStorage\Enumerations\StorageType::TEMP:
				$sTypePath = 'tmp';
				$sKeyPath = md5($sKey);
				$sKeyPath = substr($sKeyPath, 0, 2).'/'.$sKeyPath;
				break;
			case \SystemStorage\Enumerations\StorageType::USER:
				$sTypePath = 'data';
				$sKeyPath = md5($sKey);
				$sKeyPath = substr($sKeyPath, 0, 2).'/'.$sKeyPath;
				break;
			case \SystemStorage\Enumerations\StorageType::CONFIG:
				$sTypePath = 'cfg';
