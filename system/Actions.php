		...
		if ($oExc && $oExc instanceof \CApiBaseException)
		{
			switch ($oExc->getCode())
			{
				case \CApiErrorCodes::Fetcher_ConnectToMailServerFailed:
					throw new \System\Exceptions\ClientException(\System\Notifications::FetcherConnectError);
				case \CApiErrorCodes::Fetcher_AuthError:
					throw new \System\Exceptions\ClientException(\System\Notifications::FetcherAuthError);
			}
 ...
 */
		...
		{
			throw new \System\Exceptions\ClientException(\System\Notifications::MailServerError, $oException);
		}
 ...
 */
	...
	public function AjaxAccountUpdatePassword()
	{
			...
			($sCurrentIncomingMailPassword == $oAccount->IncomingMailPassword || !$oAccount->IsPasswordSpecified))
		{
			...
			catch (\Exception $oException)
			{
				...
				{
					throw new \System\Exceptions\ClientException(\System\Notifications::AccountOldPasswordNotCorrect, $oException);
				}
				
				throw new \System\Exceptions\ClientException(\System\Notifications::CanNotChangePassword, $oException);
			}
		}
	...
	public function AjaxAccountCreate()
	{
		...
		if ($oDomain)
		{
			...
			{
				$iClientErrorCode = \System\Notifications::CanNotCreateAccount;
				...
				if ($oException)
				{
					switch ($oException->getCode())
					{
						...
						case \Errs::UserManager_LicenseKeyIsOutdated:
							$iClientErrorCode = \System\Notifications::LicenseProblem;
							break;
						case \Errs::Db_ExceptionError:
							$iClientErrorCode = \System\Notifications::DataBaseError;
							break;
					}
				}
...
//		$sLang = $this->validateLang($sLang);
	...
	public function AjaxAccountRegister()
	{
		...
		if ($oAccount instanceof \CAccount)
		{
			throw new \System\Exceptions\ClientException(\System\Notifications::InvalidInputParameter); // TODO
		}
	...
	public function AjaxContactVCardUpload()
	{
		...
		if ($this->oApiCapability->isContactsSupported($oAccount))
		{
			...
			throw new \System\Exceptions\ClientException(\System\Notifications::CanNotGetContact);
		}
		else
		{
			throw new \System\Exceptions\ClientException(\System\Notifications::ContactsNotAllowed);
