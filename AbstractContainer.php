...
{
    public const SESSION_CONTAINER_PREFIX = 'sess_object_container';
    ...
    public function __set($sKey, $mValue)
    {
        ...
        if (isset($aMap[$sKey])) {
            ...
            if (\is_string($mValue)) {
                ...
                } elseif (\in_array($sKey, $this->aUpper)) {
    ...
    protected function setType(&$mValue, $sType)
    {
        ...
        } elseif (\in_array($sType, array('datetime'))) {
            \settype($mValue, 'int');
        } elseif (\in_array($sType, array('password'))) {
            \settype($mValue, 'string');
        } elseif (0 === \strpos($sType, 'string(')) {
            \settype($mValue, 'string');
            if (0 < \strlen($mValue)) {
    ...
    public static function DbUpdateArray($oObject, $oHelper, $aExclude = array())
    {
        ...
        foreach ($aMap as $sDbKey => $sObjectKey) {
            ...
            if (isset($aStaticMap[$sObjectKey][0])) {
                ...
                } elseif ('datetime' === $aStaticMap[$sObjectKey][0]) {
                    $mValue = $oHelper->TimeStampToDateFormat($mValue);
                } elseif ('serialize' === $aStaticMap[$sObjectKey][0]) {
        ...
        foreach ($aDbValues as $iIndex => $sKey) {
            ...
            if (isset($aStaticMap[$sKey][0])) {
                if ('password' === $aStaticMap[$sKey][0]) {
                    $mValue = Utils::EncryptValue($mValue);
                } elseif ('datetime' === $aStaticMap[$sKey][0]) {
                    $mValue = $oHelper->TimeStampToDateFormat($mValue);
                } elseif ('serialize' === $aStaticMap[$sKey][0]) {
                    $mValue = '' === $mValue ? '' : serialize($mValue);
                }
            }
