...
SELECT DISTINCT entity_type FROM %seav_entities',
            ...
            if (strpos($sKey, '$') !== false) {
                ...
                if (isset($mResultValue)) {
                    ...
                    if ((strtolower($mResultOperator) === 'in' || strtolower($mResultOperator) === 'not in')
                        ...
                        $sValueFormat = "%s";
                    } elseif (strtolower($mResultOperator) === 'is' || strtolower($mResultOperator) === 'is not') {
                        $bIsInOperator = true;
                        $sValueFormat = "%s";
                    } else {
    ...
    public function getEntitiesByUUID(
        $sEntityType,
        $iOffset = 0,
        $iLimit = 0,
        $aWhere = [],
...
", $aSql);
    ...
    {
        return sprintf(
            "SELECT count(id_entity) as c FROM %seav_attributes_%s
    ...
    {
        return sprintf(
            "INSERT INTO %seav_attributes_%s (`id_entity`, `name`, `value`)
    ...
    {
        return sprintf(
            "DELETE a FROM %seav_attributes_%s as a
