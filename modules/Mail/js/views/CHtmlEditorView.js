...
function CHtmlEditorView(bInsertImageAsBase64, oParent)
{
	...
	this.inactive.subscribe(function () {
		...
		if (this.inactive())
		{
			if (sText === '' || sText === '&nbsp;')
			{
				this.setText('<span style="color: #AAAAAA;">' + TextUtils.i18n('HTMLEDITOR/SIGNATURE_PLACEHOLDER') + '</span>');
				if (this.oCrea)
				{
					this.oCrea.setBlur();
				}
		...
		{
			if (sText === TextUtils.i18n('HTMLEDITOR/SIGNATURE_PLACEHOLDER'))
			{
				this.setText('');
			}
		}
...
CHtmlEditorView.prototype.onImageOver = function (oEvent)
{
	if (oEvent.target.nodeName === 'IMG' && !this.visibleImagePopup())
	{
		...
		this.tooltipText(TextUtils.i18n('HTMLEDITOR/CLICK_TO_EDIT_IMAGE'));
...
CHtmlEditorView.prototype.getNotDefaultText = function ()
{
	...
	return this.removeAllTags(sText) !== TextUtils.i18n('HTMLEDITOR/SIGNATURE_PLACEHOLDER') ? sText : '';
};
...
CHtmlEditorView.prototype.onEditorDrop = function (sUid, oData) {
	...
	if (oData && oData.File && (typeof oData.File.type === 'string'))
	{
		if (Settings.AllowInsertImage && 0 === oData.File.type.indexOf('image/'))
		{
			...
			{
				Popups.showPopup(AlertPopup, [TextUtils.i18n('COMPOSE/UPLOAD_ERROR_SIZE')]);
			}
			else
			{
		...
		else
		{
			...
			{
				Popups.showPopup(AlertPopup, [TextUtils.i18n('HTMLEDITOR/UPLOAD_ERROR_NOT_IMAGE')]);
			}
		}
	}
...
CHtmlEditorView.prototype.onFileUploadComplete = function (sUid, bResponseReceived, oData)
{
	...
	{
		Popups.showPopup(AlertPopup, [TextUtils.i18n('COMPOSE/UPLOAD_ERROR_UNKNOWN')]);
	}
};
